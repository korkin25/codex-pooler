defmodule CodexPoolerWeb.Admin.UpstreamAccountsReadModel.QuotaApiAuthorityObservationsTest do
  ...
  test "API percentage and reset remain visible with optional raw disagreement details" do
    ...
    for raw <- [[old, fresh], [fresh, old]] do
      ...
      assert html =~ "Selected first, then newest"
      assert html =~ "2% remaining"
      assert html =~ header.reset_at
      assert html =~ "data-selected=\"false\""
  ...
  test "usage parser retains normal_model_slug as metadata and separate additional scopes" do
    ...
    assert {"Reported model", reported_model} =
             List.keyfind(hd(reserve_row.observations).details, "Reported model", 0)

    assert reported_model =~ "gpt-5.6-luna"
